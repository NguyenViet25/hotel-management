...
const GuestDialog: React.FC<Props> = ({ open, initial, onClose, onSubmit }) => {
  const [guest, setGuest] = useState<GuestForm>({ name: "", phone: "" });
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      ...
      <DialogContent>
        <Stack spacing={1.5} sx={{ mt: 1 }}>
          ...
          <UploadCCCD label="Mặt trước" value={guest.idCardFrontImageUrl} onChange={(url) => setGuest({ ...guest, idCardFrontImageUrl: url })} />
          <UploadCCCD label="Mặt sau" value={guest.idCardBackImageUrl} onChange={(url) => setGuest({ ...guest, idCardBackImageUrl: url })} />
        </Stack>
      </DialogContent>
      <DialogActions>
