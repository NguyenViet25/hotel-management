...
import { Dialog, DialogTitle, DialogContent, DialogActions, Button, Grid, Stack, Typography, Divider, Chip } from "@mui/material";
...
const AssignRoomDialog: React.FC<Props> = ({ open, booking, roomType, onClose, onAssigned }) => {
  const [rooms, setRooms] = useState<RoomMapItemDto[]>([]);
  const [loading, setLoading] = useState(false);
  const [selected, setSelected] = useState<string[]>([]);

  const assignedRooms = roomType.bookingRooms || [];
  const remaining = Math.max(0, (roomType.totalRoom || 0) - assignedRooms.length);

  const fetchMap = async () => {
    setLoading(true);
    try {
      const res = await bookingsApi.getRoomMap({ date: roomType.startDate, hotelId: booking.hotelId });
      if (res.isSuccess && res.data) setRooms(res.data.filter((r) => r.roomTypeId === roomType.roomTypeId));
    } catch {}
    setLoading(false);
  };
  ...
  const confirmAssign = async () => {
    try {
      for (const roomId of selected) {
        await bookingsApi.addRoom({ bookingRoomTypeId: roomType.bookingRoomTypeId, roomId });
      }
      onAssigned?.();
      onClose();
    } catch {}
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg">
      ...
      <DialogContent>
        <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 2 }}>
          <Chip label={`Loại: ${roomType.roomTypeName || "—"}`} />
          <Chip label={`Cần gán: ${roomType.totalRoom || 0}`} />
          <Chip label={`Đã gán: ${assignedRooms.length}`} />
          <Chip label={`Còn lại: ${remaining}`} color={remaining > 0 ? "primary" : "default"} />
        </Stack>
        <Stack spacing={2}>
      ...
      <DialogActions>
        ...
        <Button variant="contained" color="primary" onClick={confirmAssign} disabled={!selected.length}>Gán Phòng</Button>
      </DialogActions>
    </Dialog>
  );
};
